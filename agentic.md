...
`OrderData` takes a string exchange while `CancelRequest.__post_init__` requires
the enum; `DDDR.encode→parse` is not self-consistent (`loads` restores objects,
the parse branch sniffs plain dicts — the upstream `fixme`); `main_contract_mapping`
keys strip digits from the whole local_symbol (`ag2612.SHFE` → `AG.SHFE`).
